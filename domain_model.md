...
- Создается по звонку клиента (возможны и другие каналы уведомления): возможны следующие варианты:
  ...
  - клиент уже в нашей базе, c этим клиентом связан заказ и этот заказ в LegacyDB - создается новый заказ в нашей базе и привязывается к этому клиенту
  - клиента нет в нашей базе но есть в LegacyDB - клиент копируется из LegacyDB в нашу базу, новый заказ создается в нашей базе и заказ привязывается к этому клиенту
